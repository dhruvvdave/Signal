...
st.markdown(
    ...
    <style>
        ...
        .signal-title { font-size: 2.4rem; font-weight: 700; letter-spacing: 0.4px; }
        ...
        .card-label { color: #9aa0a6; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.08rem; }
        .card-value { font-size: 1.5rem; font-weight: 700; color: #FAFAFA; }
...
st.markdown(
    "<div class='signal-subtitle'>Bloomberg-grade intelligence for NBA and NHL performance, fatigue, and trends.</div>",
    unsafe_allow_html=True,
)
tabs = st.tabs(["NBA Command Center", "NHL Snapshot"])

with tabs[0]:
    st.markdown("#### NBA Command Center")
    if not nba_available:
        ...
        card_cols[0].metric("Points", f"{points_avg:.1f}", points_delta)
        card_cols[1].metric("Assists", f"{assists_avg:.1f}", assists_delta)
        card_cols[2].metric("Rebounds", f"{rebounds_avg:.1f}", rebounds_delta)
        card_cols[3].metric("Minutes", f"{minutes_avg:.1f}", minutes_delta)
...
with tabs[1]:
    ...
    if nhl_standings.empty or nhl_team_stats.empty:
        ...
        nhl_summary_cols[0].metric("Teams tracked", f"{total_teams}")
        nhl_summary_cols[1].metric("Avg Goals/Game", f"{avg_gpg:.2f}")
        nhl_summary_cols[2].metric("Avg PK%", f"{avg_pk:.1f}")
